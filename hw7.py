...
class Graph(object):
    ...
    def __init__(self, adjacency_matrix):
        ...
        self.graph2 = [i[:] for i in graph]
        self.row = len(adjacency_matrix)
        self.col = len(adjacency_matrix[0])
    ...
    # Returns the minimum cut from s to t in the given graph
    def minimum_cut(self, source, target):
        ...
        for i in range(self.row):
            for j in range(self.col):
                ...
                self.graph2[i][j] > 0 and visited[i]:
                    print (str(i) + " - " + str(j))
...
mf = g.minimum_cut(source, target)
print(mf)
